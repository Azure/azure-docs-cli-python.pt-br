...
É possível instalar a CLI do Azure 2.0 com o MSI e usá-la na linha de comando do Windows ou instalar a CLI com o `apt-get` no Bash, no Ubuntu no Windows.
...
É recomendável que você instale a CLI com `apt-get` se for possível. Para as distribuições que não usam o gerenciador de pacote `apt`, você pode fazer a instalação manualmente.
...
### <a name="homebrew-on-macos-installing-older-version"></a>Instalação de versão mais antiga com o Homebrew no macOS

A fórmula `azure-cli` de Homebrew disponível para macOS está atualmente desatualizada e instalará uma versão 1.x da CLI. Você pode ver quando ela é atualizada verificando `brew info azure-cli`.

Até lá, [desinstale a versão mais antiga](#uninstall_brew) e siga as [instruções de instalação para macOS](#macOS).
...
### <a name="a-nameuninstallbrewuninstall-with-homebrew-on-macos"></a><a name="uninstall_brew"/>Desinstalar com Homebrew no macOS

Remover a CLI mais antiga com `brew uninstall`.
...
Se você tiver instalado por meio de [MSI](http://aka.ms/webpi-azure-cli) ou um [pacote de macOS](http://aka.ms/mac-azure-cli), use a mesma ferramenta para remover a instalação.
...
Execute o [MSI](https://aka.ms/InstallAzureCliWindows) novamente.
...
### <a name="uninstall-manually"></a>Desinstalar manualmente
