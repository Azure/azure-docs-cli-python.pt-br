...
Ao executar o comando `apt-key`, você verá uma saída semelhante ao seguinte erro.
...
Isso ocorre devido a um componente ausente exigido por `apt-key`. Você pode resolver isso instalando o pacote `dirmngr`.
